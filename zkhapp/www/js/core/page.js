...
angular.module('page',[])
        ...
        this.loadMore = function(scope){
            HTTP.send({
                ...
                if(data.data!=null && data.data.obj!=null){
                    ...
                    if(scope.dataList.length>0){scope.dataListLoading = 2;}else{scope.dataListLoading = 0;}
                    // 延迟这个广播对上拉加载效果提升明显
                    setTimeout(function() {scope.$broadcast('scroll.infiniteScrollComplete')}, 1000);
                }else{
                    scope.dataListLoading = -1; // 出错了
                }
            },function(e){
    ...
    .directive('pageDatalistLoading', function(){
        return{
            ...
            template: '<div class="page-data-list-loading" ng-style="{\'padding-top\':paddingTop}">'+
                      ...
                      '</div>',
            controller: function($scope, $rootScope, $cordovaToast){
    ...
    .directive('pageDatalistLoaded', function(){
        return {
            ...
            restrict: 'E',
            replace: true,
            template: '<div class="page-datalist-loaded text-center" style="line-height:25px; font-size:10px;color:#0f0f0f">已经到底了！</div>',
            controller: function($scope){
                // false：数据已加载完，true：数据未加载完
                $scope.$watch('isInfiniteScroll',function(newValue){
                    $('.page-datalist-loaded').css('display', 'none');
                    if(newValue===false && $scope.dataList!=null && $scope.dataList.length>0){
